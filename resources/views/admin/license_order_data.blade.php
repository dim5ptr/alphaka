...
<div class="content-header bg-light p-4 shadow-sm rounded" style="margin-bottom: 2%;">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0" style="color: #0077FF; font-weight: bold; font-size: 2rem;">License Orders</h1>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 d-flex flex-wrap justify-content-start align-items-center">
                ...
                <a href="{{ route('getLicenseOrderData') }}" class="btn btn-primary" style="background-color: #1c65b9; color: white; font-weight: bold; margin-right: 3%; margin-bottom: 1%;">
                    License Order
                </a>
    ...
    <table class="table custom-table mt-0">
        ...
        @foreach($data as $no => $order)
            <tr>
                ...
                <td>{{ isset($order['order_key']) ? '****' . substr($order['order_key'], -4) : 'N/A' }}</td>
                {{-- <td>{{ $order['order_key'] ?? 'N/A' }}</td> --}}
                <td>{{ $order['created_by'] ?? 'N/A' }}</td>
                <td>{{ \Carbon\Carbon::parse($order['created_date'])->format('d-m-Y H:i') ?? 'N/A' }}</td>
                <td class="action-buttons text-center">
                    <div class="btn-group" role="group" aria-label="Action Buttons">
                        <form action="{{ route('showmoredetailsadm', ['order_id' => $order['order_id']]) }}" method="POST" style="display:inline;">
                            @csrf
                            <button type="button" class="btn btn-outline-primary btn-sm custom-outline-btn" title="More details" onclick="showActivityDetails({{ $order['order_id'] }})">
...
@section('script')
<script>
    function showActivityDetails(orderId) {
    // Open the modal
    ...
    const apiUrl = 'http://192.168.1.170:14041/api/license/get_license_order_data_by_id.json';
    ...
    fetch(apiUrl, {
        ...
        body: JSON.stringify({ id: orderId }) // Passing orderId
    })
    ...
    .then(data => {
        if (data.success && data.data && data.data.length > 0) {
            ...
            document.getElementById('licenseDetailsContent').innerHTML = `
                ...
                <p><strong>License Key:</strong> ${order.order_key ?? 'N/A'}</p>
                <p><strong>Created By:</strong> ${order.created_by ?? 'N/A'}</p>
                <p><strong>Created Date:</strong> ${new Date(order.created_date).toLocaleString()}</p>
                <p><strong>Request Category:</strong> ${order.request_category ?? 'N/A'}</p>
                <p><strong>Notes:</strong> ${order.notes ?? 'N/A'}</p>
            `;
        } else {
            document.getElementById('licenseDetailsContent').innerHTML = `
                <p class="text-danger">Order details not found or unavailable.</p>
            `;
        }
    })
    .catch(error => {
