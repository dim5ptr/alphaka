...
<div class="content-header bg-light p-4 shadow-sm rounded" style="margin-bottom: 2%;">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0" style="color: #0077FF; font-weight: bold; font-size: 2rem;">Activity</h1>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 d-flex flex-wrap justify-content-start align-items-center">
                ...
                <a href="{{ route('getActivityData') }}" class="btn btn-primary me-2" style="background-color: #1c65b9; color: white; font-weight: bold; margin-right: 3%; margin-bottom: 1%;">
                    Activity
                </a>
...
<div class="table-container">
    <table class="table custom-table mt-0">
        ...
        <tbody>
            @forelse($data as $no => $activity)
                <tr>
                    <th scope="row">{{ $no + 1 }}</th>
                    <td>{{ $activity['activity_name'] ?? 'N/A' }}</td>
                    <td>{{ $activity['status'] ?? 'N/A' }}</td>
                    <td>{{ $activity['performed_by'] ?? 'N/A' }}</td>
                    <td>{{ \Carbon\Carbon::parse($activity['performed_date'])->format('d-m-Y H:i') ?? 'N/A' }}</td>
                    <td class="action-buttons text-center">
                        <div class="btn-group" role="group" aria-label="Action Buttons">
                            <form action="{{ route('showmoredetailsadm', ['activity_id' => $activity['id']]) }}" method="POST" style="display:inline;">
                                @csrf
                                <button type="button" class="btn btn-outline-primary btn-sm custom-outline-btn" title="More details" onclick="showActivityDetails({{ $activity['id'] }})">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="licenseDetailsModalLabel">Activity Details</h5>
...
function showActivityDetails(activityId) {
    ...
    const apiUrl = 'http://192.168.1.170:14041/api/license/get_activity_data_by_id.json';
    const accessToken = '{{ session('access_token') }}'; // Access token from Blade session

    // Send an AJAX request to fetch activity details
    fetch(apiUrl, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success && data.data && data.data.length > 0) {
            ...
            // Populate the modal with the activity details
            document.getElementById('licenseDetailsContent').innerHTML = `
                <p><strong>Activity Name:</strong> ${activity.activity_name ?? 'N/A'}</p>
                <p><strong>Status:</strong> ${activity.status ?? 'N/A'}</p>
                <p><strong>Performed By:</strong> ${activity.performed_by ?? 'N/A'}</p>
                <p><strong>Performed Date:</strong> ${new Date(activity.performed_date).toLocaleString()}</p>
                <p><strong>Description:</strong> ${activity.description ?? 'N/A'}</p> <!-- Assuming there's a description field -->
            `;
        } else {
            document.getElementById('licenseDetailsContent').innerHTML = `
                <p class="text-danger">Activity details not found or unavailable.</p>
            `;
        }
    })
    .catch(error => {
