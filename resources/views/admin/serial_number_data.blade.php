...
<div class="content-header bg-light p-4 shadow-sm rounded" style="margin-bottom: 2%;">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0" style="color: #0077FF; font-weight: bold; font-size: 2rem;">Serial Numbers</h1>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 d-flex flex-wrap justify-content-start align-items-center">
                ...
                <a href="{{ route('getSerialNumberData') }}" class="btn btn-primary" style="background-color: #1c65b9; color: white; font-weight: bold; margin-right: 3%; margin-bottom: 1%;">
                    Serial Number
                </a>
            </div>
...
<div class="table-container">
    <table class="table custom-table mt-0">
        ...
        <tbody>
          @foreach($data as $no => $serialNumber)
                <tr>
                    <th scope="row">{{ $no + 1 }}</th>
                    <td>{{ $serialNumber['serial_id'] ?? 'N/A' }}</td>
                    <td>{{ isset($serialNumber['activation_key']) ? '****' . substr($serialNumber['activation_key'], -5) : 'N/A' }}</td>
                    {{-- <td>{{ $serialNumber['activation_key'] ?? 'N/A' }}</td> --}}
                    <td>{{ $serialNumber['is_used'] ? 'Yes' : 'No' }}</td>
                    <td>{{ $serialNumber['activated_date'] ? \Carbon\Carbon::parse($serialNumber['activated_date'])->format('d-m-Y H:i') : 'N/A' }}</td>
                    <td>{{ \Carbon\Carbon::parse($serialNumber['created_date'])->format('d-m-Y H:i') ?? 'N/A' }}</td>
...
<script>
    ...
    const apiUrl = 'http://192.168.1.170:14041/api/license/get_serial_number_data_by_id.json';
